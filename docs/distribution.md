...
`pnpm release:pack` 生成 `artifacts/admin-kit-0.1.0.tar.gz`，可作为 GitHub Release 附件。解压后运行 `pnpm install`，再执行 `node packages/cli/index.mjs create my-admin`。包内包含模板、工具源码、规则和说明；新项目可以独立运行。
