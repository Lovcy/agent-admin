...
1. Read `.admin-kit/rules/core.md`, `admin-kit.config.json` and the relevant module. Run `pnpm doctor`. A source connector is needed only when that source is requested.
...
5. Implement in the existing layers. Use `.admin-kit/skills/admin-frontend/SKILL.md` for Vue and UI work. Keep the plan and task status current. Record unresolved requirements with `pnpm task block <id> <reason>`.
